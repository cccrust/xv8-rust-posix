...
echo "  _fs _pipe _proc _fd _sbrk _cow _syscall _thread _thread_v3 _eventfd _memfd_create _pidfd _splice _getrandom _close_range _inotify _signalfd _timerfd"
echo ""
...
touch /tmp/testmode
echo "fs,pipe,proc,fd,sbrk,cow,syscall,thread,thread_v3,eventfd,memfd_create,pidfd,splice,getrandom,close_range,inotify,signalfd,timerfd" > /tmp/test_args
...
for name in fs pipe proc fd sbrk cow syscall thread thread_v3 eventfd memfd_create pidfd splice getrandom close_range inotify signalfd timerfd; do
