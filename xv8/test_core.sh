...
echo "  _fs _pipe _proc _fd _sbrk _cow _syscall _thread _thread_v3"
echo ""
...
touch /tmp/testmode
echo "fs,pipe,proc,fd,sbrk,cow,syscall,thread,thread_v3" > /tmp/test_args
...
for name in fs pipe proc fd sbrk cow syscall thread thread_v3; do
