...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: uncompress <file.Z>...");
        std::process::exit(1);
    }
    for file in &args[1..] {
        if !file.ends_with(".Z") && file != "-" {
            eprintln!("uncompress: {}: unknown suffix", file);
            std::process::exit(1);
        }
        let data = if file == "-" {
            let mut buf = Vec::new();
            ...
            std::fs::read(file).unwrap_or_else(|e| {
                eprintln!("uncompress: {}: {}", file, e);
                std::process::exit(1);
            })
        };
        let decompressed = lzw_decompress(&data);
        let out_name = if file == "-" { "stdout" } else { &file[..file.len()-2] };
        if file == "-" {
            io::stdout().write_all(&decompressed).ok();
        } else {
            std::fs::write(out_name, &decompressed).unwrap_or_else(|e| {
                eprintln!("uncompress: {}: {}", out_name, e);
                std::process::exit(1);
            });
        }
    }
